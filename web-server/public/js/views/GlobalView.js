  ...
  var View = Backbone.View.extend({
    ...
    bindGlobalEvents: function() {
      ...
      pomelo.on("onUpdateMyself", function(data) {
        ...
        me.options.game.session.user.chips =
          data.user.chips || me.options.game.session.user.chips;
        me.options.game.session.user.username =
          data.user.username || me.options.game.session.user.username;
    ...
    renderMessages: function() {
      ...
      this.messageArea.html(
        _.template($("#ChatMessageList").html(), {
        ...
        me.options.game.sendMessage(msg.val(), me.selectedFriend, function(e) {
          if (e) {
            me.addMessage(
              me.selectedFriend,
              "System",
              e == "user-not-online" ? "user is not online." : e,
              null,
              "error"
            );
          }
    ...
    addMessage: function(context, username, text, time, type) {
      ...
      this.messageArea.append(
        _.template($("#ChatMessageItem").html(), message)
      );
    ...
    renderUserList: function(dom, friends, showCheckbox) {
      dom.html(
        _.template($("#FriendListTmpl").html(), {
          friends: friends,
